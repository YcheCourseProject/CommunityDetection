        ...
        if [ -d $1"/"$file ];then
            ...
            if [[ $file =~ .*input.csv$ ]];then
                ...
                do
                    ...
                    output_attractor_new_rmtailor_suffix="_attractor_new_rmtailor_"$lamda_str".clu"
                    output_connected_componets_attractor_new_rmtailor_suffix="_attractor_new_rmtailor_"$lamda_str".cc"
                    ...
                    window_size="10"
                    attractorReduceEI -A $lamda_str 0 $init_infile $clu_out_file $components_outfile  $window_size       
                    endTime=$(date +%s.%Nns)
                    ...
                    if [[ $truth_file_num == "1" ]];then
                        if [[ $truthfile =~ .*amazon.* ]];then
                            echo "amazon"
                            measurement_out_file=${init_infile//edges_input.csv/measurement_attractor_new_rmtailor_$lamda_str.us}
                            attractorReduceEI -E US $clu_out_file $init_infile $measurement_out_file
                        else
                            echo "s"
                            ground_truth_file=$1"/"$truthfile
                            measurement_out_file=${init_infile//edges_input.csv/measurement_attractor_new_rmtailor_$lamda_str.s}
